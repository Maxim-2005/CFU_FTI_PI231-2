		...
		for x := 0; x < 3; x++ {
			if board[y][x] == b.marker {
				...
				if (board[y][0] == " " || board[y][0] == b.marker) &&
					...
					for i := 0; i < 3; i++ {
						if board[y][i] == " " {
							return y, i, NewRandomBot().marker
						}
					}
				}
				// вертикаль
				if (board[0][x] == " " || board[0][x] == b.marker) &&
					...
					for i := 0; i < 3; i++ {
						if board[y][i] == " " {
							return i, x, NewRandomBot().marker
						}
					}
				}
				// диогональ
