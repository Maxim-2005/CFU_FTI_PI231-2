...
int main()
{
    ...
    std::vector<int> max_score_Under70_results;
    ...
    for (const auto& doc : collection.find({})) {
        ...
        if (has_valid_scores) {
            ...
            if (is_under_19 && is_max_score_under_70) {
                max_score_Under70_results.push_back(scores.max_score);
            }
        }
    }
    ...
    std::transform(
        max_score_Under70_results.begin(),
        max_score_Under70_results.end(),
        std::ostream_iterator<int>(std::cout, " "),
        [](int score) { return score; }
    );
    std::cout << "\n";
